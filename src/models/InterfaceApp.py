...
from src.utils.constants import ROBOTO_14
from src.utils.constants import TEXT_YOUR_MESSAGE
...
class InterfaceApp:
    ...
    def __create_widgets(self) -> None:
        ...
        text_label = Label(self.root, font=(ROBOTO_14), text=TEXT_YOUR_MESSAGE, bg=PRIMARY, fg=WHITE)
        text_label.place(x=20, y=10)

        text_entry = Entry(self.root, font=(ROBOTO_14), textvariable=self.entry_text_user)
        text_entry.place(x=180, y=10)

        action_button = Button(self.root, font=(ROBOTO_12), text="Listen", command=lambda:self._perform_speech(), bg=PRIMARY, fg=WHITE)
        action_button.place(x = 450, y = 5)

        self._combo = ttk.Combobox(state="readonly", values=list(self.engine.voices.keys()), width=40)
